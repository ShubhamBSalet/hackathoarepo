...
//         alert("Your details do not match our records.");
//       } else if (response.ok) {
...
export default function SignUp() {
  ...
  // Function to check if the voter ID, mobile number, and email exist in GovernmentData
  const checkGovernmentData = async () => {
    try {
      const response = await fetch(`http://localhost:8000/check-government-data?voterId=${voterId}&mobileNumber=${mobileNumber}&email=${email}`);
      if (response.ok) {
        const data = await response.json();
        if (data.exists) {
          setGovernmentDataValid(true);
          alert("Your details match our records. You can proceed with the signup.");
        } else {
          setGovernmentDataValid(false);
          alert("Your details do not match our records. Please check your Voter ID, Mobile Number, and Email.");
        }
      } else {
        setGovernmentDataValid(false);
        alert("Error checking government data. Please try again.");
      }
    } catch (error) {
      console.error("Error:", error);
      setGovernmentDataValid(false);
  ...
  const handleSubmit = async (event) => {
    ...
    if (!governmentDataValid) {
      alert("Please verify your details with the government records first.");
      return;
    }
          ...
          {isOtpSent ? (
            ...
            <Form onSubmit={handleSubmit}>
              ...
              <Button
                ...
                type="submit"
                className="w-100"
                disabled={!isFormValid || !governmentDataValid}
              >
                Register
              </Button>
              ...
              <Button
                ...
                onClick={checkGovernmentData}
              >
                Verify Government Data
              </Button>
            </Form>
          )}
